...
public class PurchaseImp implements PurchaseInterface {
	...
	public List<PurchaseRespDTO> getAllPrducts() {
		List<Purchase> entityList = purchasedao.findByIsDeletedFalse();
	...
	public boolean deleteById(int purchaseId) {
		Optional<Purchase> optionalPurches = purchasedao.findById(purchaseId);
		if (optionalPurches.isPresent()) {
			Purchase product = optionalPurches.get();
